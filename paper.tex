...
The new manager gives them a challenge: ``Here are your login credentials for our cloud provider services. Here's a few hundred gigabytes of messy data that nobody has yet had time to look at. Familiarize yourself with it and find us some insights.''
...
We designed and taught a class whose implicit goal is to prepare students for the scenario described above, and this paper contains some of the key insights and experience we've acquired along the way.
...
Our department has offered one section of Stat 129 each Spring semester since 2021.
...
This process works great if the data fits into memory, but if the data does not fit into memory, then it doesn't work at all, because it fails at the loading step.
...
There is a qualitative difference between running code on a laptop with a monitor that's sitting in front of you, and on a machine that you never actually see, that you can only access through \texttt{ssh}, secure shell, a purely text based way of interacting with a remote computer.
...
One topic that we cover in Stat 129 is reading the output of \texttt{top/htop} to identify a bottleneck in a \texttt{bash} pipeline that's streaming hundreds of GB of data.
...
We've used a cluster with SLURM \cite{yoo2003slurm} job queueing software, common in high performance computing (HPC), for teaching Stat 141C (Big Data and High Performance Computing) at University of California, Davis in 2019.
...
The server rack option is generally preferable, as the machines have more central processing units (CPU) and memory, better internet connectivity, and they don't clutter up your space.
...
We recommend against setting up a physical server at your home, because providing reliable remote access could be tough depending on your internet provider, and there's no reason you should pay for the electricity.
...
There are no limits on how much computational time a user can use, so it's possible for one user to use up all the credits for the allocation.
...
A sustainable funding model is to make students pay for commercial cloud services, and this doesn't need to be expensive.
...
\item \textbf{long term sustainability} 
    ...
    A 1 year pilot program might be a useful start, but it's much more compelling to have a guarantee that a server will be available for teaching for the next 5 years.
