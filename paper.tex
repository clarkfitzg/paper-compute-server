...
%\author{Clark Fitzgerald, Assistant Professor \protect\\
%phone: 916-278-4748, email: \href{mailto:fitzgerald@csus.edu}{fitzgerald@csus.edu} \protect\\ 
%Department of Mathematics and Statistics \protect\\ 
%California State University, Sacramento \protect\\ 
%6000 J Street, Sacramento, CA 95819, United States
%}
\begin{document}

%X The title of the manuscript.
%X Details of all co-authors, including their full names, departments, institutions, cities, and countries of residence.
%X Identity and complete contact details of the corresponding author, including his/her full name, postal and e-mail addresses, and telephone and fax numbers.
%X five to six keywords or phrases suitable for use in an index


\maketitle

keywords: big data, cloud computing, data science, education, pedagogy, statistical computing
...
Imagine yourself as a student who has just graduated college with an undergraduate degree in applied math or statistics, and started their first job working with data.
The new manager gives them a challenge: ``Here are your login credentials for our cloud provider services. Here are a few hundred gigabytes of messy data that nobody has yet had time to look at. Familiarize yourself with it, and find us some insights.''
This is certainly a tall order, but the graduate who has the ability to even \emph{begin} such a task is valuable and rare indeed.
Success requires technical computing skills, sound statistical reasoning, and an ability to `think with data'.

The implicit goal of STAT 129, a class in `Big Data' at California State University, Sacramento, is to prepare students for the scenario described above.
This paper contains some of the key insights and experience from developing and teaching this course for the past five years.
Section~\ref{sec:stat129} describes the class, including what kind of data suits our pedagogical purpose and why we need a compute server.
Section~\ref{sec:serveropt} provides an overview of the role of the server in our class, and discusses the two physical server options: institutionally supported and personal.
Section~\ref{sec:cloud} covers cloud server options, including publicly funded and commercial.
Section~\ref{sec:conclusion} concludes with our `wish list' for an ideal shared educational compute service that will allow many instructors to simultaneously teach similar classes without the overhead of administering their own server.


\section{The class: STAT 129}
\label{sec:stat129}

STAT 129, titled `Analyzing and Processing Big Data', is an upper division elective course offered by the Mathematics and Statistics Department at California State University, Sacramento, and is required for students pursuing a bachelor's degree in statistics with an emphasis in data science.
...
STAT 129 requires a prerequisite class in programming, and the most successful students came into the class with strong programming skills.
...
The philosophy behind STAT 129 is to provide students with real world data sets and ask open ended questions, following in the footsteps of Deborah Nolan and Duncan Temple Lang \cite{nolan2010computing}.
We are statistics faculty with a background in computing, and we've developed and taught STAT 129 each Spring semester since 2021.
...
%Our department has offered one section of 
Enrollment is capped at 25 students, and STAT 129 has enjoyed strong enrollment, with 18-25 students per year for the last three years.
Feedback from our data forward approach has been overwhelmingly positive, with several alumni reporting STAT 129 as the most practical class they have taken in college.
...
Conventionally, big data refers to large, unwieldy data sets that require some effort to analyze \cite{sagiroglu2013big}.
For the purposes of STAT 129, `big data' has a precise meaning: a data set that is large enough such that one cannot load the entire data set into a laptop's memory at once.
In a typical introductory data analysis class, a student loads a single data file into memory and proceeds with the analysis steps \cite{wickham2023r}.
This process works great if the data fits into memory, but if the data does not fit into memory, then it does not work at all, because it fails at the loading step.
...
Our goal is to choose data sets that are comfortably over this memory threshold, such that the techniques we are learning are motivated, but not gratuitously large such that answering a basic question takes an excessive amount of time.
...
We assign data analysis tasks that typically take 1 to 10 minutes if done sequentially, and are an order of magnitude faster if done in parallel.
...
\begin{enumerate}
\item \textbf{Size:} The files should be large enough to motivate the computational techniques taught in class, but not excessively large, as described above.
...
\item \textbf{Format:} 
    ...
    We spend several weeks working with unstructured text, which we usually extract as a field inside one of the structured formats above.
    The data is usually compressed, with one data set stored across multiple files.
...
\item \textbf{Background Knowledge:}
    ...
    For example, in the Global Historical Climatology Network-Daily Database \cite{menne2012overview}, each row represents one weather observation for a specific date and location, which is easy to explain.
\end{enumerate}
The above criteria do not necessarily apply to more domain specific classes. 
...
Amazon Open Data \cite{aws-opendata} has provided a reliably good source of large data sets fitting these criteria.
The US government \cite{data-gov} is another good source.
...
We often find interesting data sets that are close, but not quite what we are looking for.
...
We removed the XML namespace information in these files so that students learning XPATH can write:
\begin{verbatim}
//ActivityOrMissionDesc
\end{verbatim}
to find the `activity or mission description' for a nonprofit rather than the cumbersome:
\begin{verbatim}
...
Students must access data and run code on remote servers for a class on `Big Data', because that is where big data is typically found: on remote servers.
There is a qualitative difference between running code on a laptop with a monitor that is sitting in front of you, and on a machine that you never actually see, that you can only access through \texttt{ssh}, secure shell, a purely text based way of interacting with a remote computer.
After opening a terminal and logging in to the remote server with \texttt{ssh}, all you see is a \texttt{bash} prompt waiting for you to type commands; this is the command line.
For example, the following shows what it would look like after the user Jane Doe logged in to the remote server via \texttt{ssh} and entered the \texttt{pwd} command to print her working directory:
\begin{verbatim}
jane@nsm-stats:~$ pwd
/home/jane
jane@nsm-stats:~$
\end{verbatim}
Our goal with embracing the command line is to move students beyond the initial unfamiliarity and discomfort that users experience when they first move away from a Graphical User Interface (GUI).
...
The command line is the only way for STAT 129 students to access the large data sets and complete the assignments, quizzes, and exams.
...
%Students in technology oriented class such as STAT 129 benefit when instructors focus on ubiquitous and proven core technologies, rather than on what happens to be popular this year, because then they come away with skills that they can apply in many different contexts.
%If one hopes to do anything high performance computing (HPC) or frameworks such Hadoop MapReduce \cite{white2012hadoop}, then a foundational working knowledge of the command line is essential.
...
One topic that we cover in STAT 129 is reading the output of \texttt{top/htop} to identify a bottleneck in a \texttt{bash} pipeline that is streaming hundreds of GB of data.
It is possible to do this with a cluster, but it takes either more advanced knowledge, or tools specific to that system.
...
Which server option is best depends on the goals of the course, the level of institutional support, the number of students enrolled and their background, as well as the instructor’s preferences.
...
In a given year we will have one instructor, 15-30 students, and no staff, teaching assistants, or other support.
We do not need a large machine for any purpose other than teaching.
...
We have yet to discover a perfect solution, and what is best for you may well differ from our preferences.
...
If you are in the lucky position of having compute infrastructure with dedicated support in your department, college, or university, then this is an ideal option for teaching classes.
...
We have used a cluster with SLURM \cite{yoo2003slurm} job queueing software, common in high performance computing (HPC), for teaching Stat 141C (Big Data and High Performance Computing) at University of California, Davis in 2019.
...
\item Setting up and configuring an open source Postgres database for students.
\end{itemize}
...
If you do not have institutional support, then it is possible to be your own systems administrator, and we consider this situation next.


\subsection{Personal Physical Server}

A personal physical server is essentially the same as a powerful desktop machine, except that you can access it remotely, and it does not need a monitor.
The machine itself might be in a rack in a server room on campus, or in someone's office.
The server rack option is generally preferable, as the machines have more central processing units (CPU) and memory, better internet connectivity, and they do not clutter up your space.
...
Regarding cost, we have found it easier to fund a one time purchase of a few thousand dollars, compared to finding a sponsor who will commit to the ongoing variable subscription fees of the cloud services, even if the total cost would be lower for the cloud subscription.
Physical servesr are simple because you can set it up one time, and then use the same machine for several years.
We did not need any specialized knowledge beyond the basics.
We completely own and administrator the machine, so we can install whatever we want, and whenever we want.
It is relatively easy and direct to automate certain grading tasks, for example, an instructor can use administrative privileges to look at \texttt{history} to see every command a student typed during a quiz.
...
The primary disadvantage of a single physical machine is that if it fails, then the class effectively stops.
If students cannot use the server, then they cannot do their homework, and they cannot do quizzes.
If you are managing the server, then you must identify and fix the problem.
...
We are taking a risk that the server could fail in a more serious way, but it is a calculated risk.
...
The server does not need to be powerful or expensive.
...
We always recommend that students do their homework well before the due date, because if a handful of users are simultaneously trying to use parallel jobs with all available CPU cores, then they will not see the speedup they expect.
In practice, it happened occasionally that students who waited until the last hour to do an assignment did not get to see their final result, but they were not surprised, because we had warned them many times and they knew to expect this.
We have used a 16 core CPU machine in previous semesters, and found that it was not sufficient for 15 students.
...
Our students need to be either on campus or at home on the campus virtual private network (VPN), and they can login via \texttt{ssh}.
We recommend against setting up a physical server at your home, because providing reliable remote access could be tough depending on your internet provider, and there is no reason you should pay for the electricity.
...
From a student's perspective, there is no difference between logging in via \texttt{ssh} to a physical machine, or to a virtual machine running on the cloud.
...
We have experimented with two ways of providing students with cloud access: local user accounts for each student on a VM, and administrative cloud accounts for every student.
...
They are the administrators to these machines with \texttt{sudo} access, so they can do anything they wish, and any mistakes they make will not affect anyone else.
Students can create machines that are as large as they like, and delete them when they are done, which solves the problem of many students trying to simultaneously use limited resources on a single machine.
%When many students are on the same machine, and they're all simultaneously trying to use all the available CPU's right before the deadline for homework submission, this can create load balancing problems.
...
In this scenario, the instructor creates a VM that is large enough to accommodate all of the students simultaneously, and students access through \texttt{ssh} or a web interface.
...
The Jetstream2 website \cite{jetstream-web} states:
\begin{quote}
...
Thanks to NSF funding, our students, many of whom are low income, received accounts without providing a credit card.
The user interface shown in Figure~\ref{fig:jetstream} is simple and easy to use, with a manageable number of options.
...
This user interface is simple and easy to teach with.}
    \label{fig:jetstream}
\end{figure}
...
There are no limits on how much computational time a user can use, so it is possible for one user to use up all the credits for the allocation.
In other words, the only thing that is stopping them from doing something bad is their own awareness and respect for others, which is not ideal for a group of 20-30 novices.
...
We were not monitoring as diligently as we should have, and this used up a significant portion of our available compute time, but we still had plenty to get through the course.
...
Annual renewal is not an issue if someone is paying for commercial cloud services, and we turn our attention here next.
...
Sustainable ways to provide servers for teaching a class like STAT 129 are essential, as discussed in Section~\ref{sec:serveropt}.
A sustainable funding model is to make students pay for commercial cloud services, and this does not need to be expensive.
...
In the past, AWS offered an educational program with free credits for students, which did not even require a credit card.
...
Indeed, the main purpose of the `AWS Cloud Practitioner' certification seems to be to understand all of these proprietary names and services.
...
Critiques aside, commercial cloud services still offer an appealing option for teaching STAT 129.
With the commercial cloud you can get started very quickly with minimal work and investment.
...
None of the available cloud services that we have explored or mentioned have proven to be ideal for teaching STAT 129.
We conclude by describing what \emph{would} be ideal, based on our experience.

\section{Moving Forward}
\label{sec:conclusion}

There are many technical details and considerations when relying on a server to teach an undergraduate data analysis class like STAT 129, and we have only provided a broad overview here.
...
\item \textbf{free or low cost} Free for students is ideal.
...
\item \textbf{long term sustainability} 
    ...
    A 1 year pilot program might be a useful start, but it is much more compelling to have a guarantee that a server will be available for teaching for the next 5 years.
\item \textbf{minimal specificity}
    It is ideal if students in a one semester data science class do not need to learn concepts specific to one system or vendor, as discussed in Sections~\ref{sec:server} and~\ref{sec:comcloud}.
...
Some specific things we can do are: encourage our peers to accept the challenge of teaching material that may be outside of their comfort zones, share our course materials, and meet with peers regularly when they are teaching this new material for the first time.

Large data sets are here to stay, and we serve our students well by teaching them how to actually work with large data.
It is quite uncommon to see a course designed around the student experience on a server, particularly in a mathematics and statistics department.
